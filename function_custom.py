...
def get_kelurahan_data(url, city_name, province):
    ...
    if table:
        ...
        for i, header_column in enumerate(header_columns):
            if 'Daftar Desa' in header_column.text or 'Daftar Desa/Kelurahan' in header_column.text or 'Daftar Kelurahan' in header_column.text or 'Daftar Gampong' in header_column.text or 'Daftar' in header_column.text:
    ...
    data = {'kelurahan': kelurahan_list, 'kota': city_name, 'provinsi': province}
    return pd.DataFrame(data)

# Fungsi untuk menghapus karakter non-latin
def hapus_karakter_non_latin(teks):
